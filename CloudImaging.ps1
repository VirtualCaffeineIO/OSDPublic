...
New-OSDCloudWorkspace -workspacepath $MyWorkspace

#Get Autopilot profile and download it to the OSDCloud library
$creds = Get-Credential
Connect-MSGraph -Credential $creds
Get-AutopilotProfile | Where-Object DisplayName -eq "Autopilot OSDCloud Pro" | ConvertTo-AutopilotConfigurationJSON | Out-File -FilePath "$($MyWorkspace)\AutoPilot\Profiles\AutoPilotConfigurationFile.json" -Encoding ASCII
...
Edit-OSDCloud.winpe -workspacepath $MyWorkspace -CloudDriver VMware
<#
...
New-OSDCloudIso -workspacepath $MyWorkspace
